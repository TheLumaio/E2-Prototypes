...
void draw()
{
            ...
            if (world_x+(chunk_x*16) == mpos.x+_camera.off_x-1 && world_y+(chunk_y*16) == mpos.y+_camera.off_y-2)
            {
                ...
                if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON))
                {
                    ...
                    if (chunk->ents[world_y*WORLD_CHUNK_SIZE+world_x] == NULL)
                        chunk_add_ent(chunk, ent_lightning(chunk, world_x, world_y, 3));
                }
            }
    ...
    for (int i = _log.scroll; i < _log.log->entries; i++)
    {
        ...
        e2_rich_print((char*)str->data, 1, 49+i-_log.scroll, 0x07);
    }

    for (u8 a = 0; a <= 0xf; a++)
    {
        e2_putc(0xdb, 51, 3 + a, a | (0xf-a) << 4);
        e2_print(FormatText("%02x", a), 52, 3 + a, 0x07);
    }
